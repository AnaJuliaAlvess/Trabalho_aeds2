...
int main(){
    ...
    while(op!=-1){
        switch (op){
            ...
            case (6):
                if(crioulistadenomes){
                    printf("PARTE 1 SELEÇÃO NATURAL");
                    nParticoes= 0;
                    selecao_natural(arq, nomes, nElementos, nFunc, &nParticoes);
                    //salvando o ponteiro para o início da lista de nomes
                    prox = nomes;
                    ordenacao_part(nomes, nParticoes, nFunc);
                    imprimirFuncs(nomes, nParticoes, nFunc);
                    criouparticoes=1;
